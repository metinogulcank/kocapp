...
-- Not: Bu tabloları oluşturmadan önce 'ogrenciler' ve 'users' tablolarının mevcut olduğundan emin olun
CREATE TABLE IF NOT EXISTS ogrenci_programlari (
    ...
    CONSTRAINT fk_ogrenci_program_ogrenci FOREIGN KEY (ogrenci_id) 
        REFERENCES ogrenciler(id) ON DELETE CASCADE,
    CONSTRAINT fk_ogrenci_program_ogretmen FOREIGN KEY (ogretmen_id) 
...
CREATE TABLE IF NOT EXISTS program_sablonlari (
    ...
    guncelleme_tarihi DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    INDEX idx_ogretmen (ogretmen_id),
    CONSTRAINT fk_sablon_ogretmen FOREIGN KEY (ogretmen_id) 
        REFERENCES users(_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS sablon_program_detaylari (
    ...
    CONSTRAINT fk_sablon_detay_sablon FOREIGN KEY (sablon_id) 
